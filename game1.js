 ...
 */
 ...
 // Game variables
        ...
        function saveScoreToDatabase() {
            ...
            if (scoreSaved) {
                showSaveMessage('Score already saved', false);
                return;
            }
            
            // Use AJAX to send score to server
            ...
            xhr.onload = function() {
                if (xhr.status === 200) {
                    ...
                    if (response === 'success') {
                        showSaveMessage('Score saved successfully!', true);
                        scoreSaved = true;
                        // Reload high scores
                        loadHighScores();
                    } else {
                        showSaveMessage('Error saving score: ' + response, false);
                    }
                } else {
                    showSaveMessage('Error connecting to server', false);
                }
            };
            ...
            // Send score data
            xhr.send(`playerName=${encodeURIComponent(playerName)}&score=${score}`);
        }
        
        // Display save message
        function showSaveMessage(message, isSuccess) {
            saveMessage.textContent = message;
            saveMessage.className = isSuccess ? 'save-success' : 'save-error';
        ...
        function loadHighScores() {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', 'getHighScores.jsp', true);
        ...
        // Save score button
        saveScoreBtn.addEventListener('click', saveScoreToDatabase);
        
        // Player name input - allow saving on enter key
